...
4. **In-flight worktrees** — worktrees under `.claude/worktrees/` are listed by path, and the first recommendation is to *resume* one: `EnterWorktree(path="…")` puts this session back inside it (paths below `.claude/worktrees/` need no approval), from where `/ticket-flow:status` and `/ticket-flow:implement` work normally. That is the recovery entry point after a subagent died — its branch still holds the work. Only after `git merge-base --is-ancestor` confirms the branch is merged is `git worktree remove` the right move; removing first throws the work away.
