...
- **Everything else**: dispatch **one** `Agent` call, `subagent_type: "general-purpose"`, `model: "haiku"`. Embed the full diff in the prompt yourself (`git diff <target-branch>...HEAD`) — the reviewer checks the diff it's handed, it does not go collect one itself. **Run the command and paste its output into the prompt text.** A prompt string is never shell-expanded: a literal `$(git diff …)` or `$(cat …)` arrives at the subagent as those characters, and a reviewer handed no diff will review the surrounding prose instead and report that as findings — a fabricated review that reads exactly like a real one (observed 2026-08-25, caught only because the dispatching agent noticed and re-ran it). Ask it to check for correctness bugs, contradictions with the surrounding code/doc text, and call sites or other references the change may have missed. Wait for its result and act on the findings before writing the verdict.
...
**A lock is a different error from `Operation not permitted`** and is never ambiguous: `git worktree remove` refuses a locked worktree outright (`fatal: cannot remove a locked working tree`, git 2.54) and a single `--force` does not override it; `-ff` does.

**Judge a lock by whether the work is safe, not by whether the locking pid is alive.** Claude Code locks a running agent's worktree and releases the lock when that *process* exits — and the process is the whole session, not the individual agent. After a subagent has reported back, the locking pid is therefore still alive, so a pid check alone never clears anything and the worktrees pile up until the session ends (measured 2026-08-25: two of four dispatched worktrees stranded that way). The lock exists to stop a *concurrent* cleanup from pulling a worktree out from under a working agent — once the agent has reported, there is no concurrency left to protect against. So: agent reported **and** `git merge-base --is-ancestor <branch> <target>` passes **and** `git -C <path> status --porcelain` is empty → `git worktree unlock <path>` then `git worktree remove <path>` (or `git worktree remove -ff <path>`), regardless of the pid. Any of those three missing → leave it standing. Claude Code also sweeps its own agent worktrees periodically (`cleanupPeriodDays`, skipping ones that still hold changed files or unpushed commits, and releasing only locks it set itself), so a stranded worktree is untidiness, not data loss.
