...
> Read-only snapshot generated by \`/ticket-flow:board\` from bd state — NOT a workflow input. Edit bd, then re-run \`/ticket-flow:board\`. The intake zone (\`<!-- INTAKE:START -->\` … \`<!-- INTAKE:END -->\`) is preserved verbatim across regenerations.
