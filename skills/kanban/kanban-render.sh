#!/usr/bin/env bash
# kanban-render.sh — generate a read-only KANBAN.md snapshot from bd state.
#
...
#
# SCOPE (spec #23): this is NOT a workflow step. In beads mode (`mode=beads`)
# no workflow skill reads or writes KANBAN.md; this renderer is reachable only
# via the manual `/ticket-flow:board` command, which produces an on-demand
# static board snapshot. The snapshot is an output, never a workflow input.
#
# - KANBAN.md is fully generated output; new items go straight to `bd create`.
#   (The former hand-written intake zone was removed with mode=kanban — see
#   refs/archive/mode-kanban.)
#
# Usage:
...
> Read-only snapshot generated by \`/ticket-flow:board\` from bd state — NOT a workflow input. Edit bd, then re-run \`/ticket-flow:board\`.
...
case "$MODE" in
  ...
  write)
    ...
    echo "✓ wrote KANBAN.md ($LINES lines)"
    ;;
  check)
    TMP="$(mktemp -p /tmp/claude 2>/dev/null || mktemp)"
    emit > "$TMP"
