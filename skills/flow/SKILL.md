...
A subagent can die *before* delivering its report, while a usable diff already sits in its worktree — or it can go quiet without dying at all: the harness still shows it running, but no new tool output has appeared for a long stretch. **Read the situation first — the three cases need different responses, and the first two are distinguishable from the third by one thing: they come with an explicit error message. A stall does not — it is silence, nothing else.**
...
- **Spend limit** ("you've hit your monthly spend limit") or out of credits: **read the message before classifying it.** When it **names a reset time** ("your session limit resets HH:MM"), it is the plan's *time* quota with credit-overage disabled — the window reopens by itself; treat it exactly like a usage limit (wait, then resume per rule 1). Only a spend-limit message **without** a reset time is the hard money ceiling: it does not reset, the user has to raise it, and waiting never helps — which is why persistent retry mode fails immediately on these instead of polling (2.1.239). In `--serial`/`--loop`, on the hard case: **stop the run**, report which tickets are done and which are untouched, and say plainly that the limit needs raising. Re-dispatching against a money ceiling produces a chain of instant deaths that burns wall-clock and context for nothing.
- **Stall** (no error, no death — just quiet): judged by comparing elapsed wall-clock time since dispatch to what a normal dispatch on this project looks like. There is no fixed number here and tf does not compute one: it measures and reports elapsed time, it never decides "stalled" on its own — a guessed cutoff would be indistinguishable from a clean run right up until it wrongly kills one (the decision this ticket is built on, 2026-08-25). For orientation only — **an observation from one real run, not a threshold** (DSP autopilot swarm, 19 subagents, 2026-08-25): ordinary dispatches finished in 1–7 minutes; seven agents died on the usage limit, five of them at exactly 23 minutes; one outlier finished normally at 187 minutes. Note the dispatch time per ticket (e.g. `date -u +%FT%TZ` right before the `Agent` call) and carry elapsed time into the P5/P7 report next to that ticket — the controller (the user in `--local`/`--parallel`, the caller's own policy layer in `--serial`/`--loop`) judges from there whether it is worth a `SendMessage` probe or more waiting. Probe before treating a stall as dead: a quiet agent can still be mid-reasoning or running a slow tool, and `SendMessage` to a live agent is harmless.

Then, for a death of either limit kind, or a stall confirmed unresponsive after a probe, two rules:
...
0. **Verdict gate** (mandatory, before anything else): save the subagent's final message to a file and run `"${CLAUDE_PLUGIN_ROOT}/skills/flow/verdict-check.sh" <file>` — it extracts the fenced ```json verdict, validates it (branch, git sha, non-empty `acs` with `proven|residual` each, proven needs evidence, `tests` present) and prints `BRANCH=… SHA=… PROVEN=… RESIDUAL=… BLOCKERS=… REVIEW=…` for `eval`. **Invalid or missing verdict → do not merge on prose.** Treat it like P4a rule 2: inspect the worktree diff, then `SendMessage` the subagent asking for the verdict (worktree still present) or dispatch fresh (worktree gone). `BRANCH`/`SHA` feed step 1, `PROVEN`/`RESIDUAL` feed the gating in step 4, `BLOCKERS>0` goes to step 5, `REVIEW` goes verbatim into the P5/P7 report (it is reported, never gated — but a `not run (…)` or `not reported` is stated, not swallowed). Pattern: Castra's persona verdict — nothing mutates until the verdict validates.
    ...
    **A shipped hook makes this structural, not just procedural.** `hooks/verdict-gate.sh` runs on `SubagentStop` and validates the same message through the same script *before the agent is gone*, handing back the defect list while its context still exists — so a missing verdict usually costs one extra turn instead of a re-dispatch. It fires only inside a `.claude/worktrees/` checkout of a ticket-flow project, and blocks at most once per agent, so an agent that genuinely cannot produce one still ends. The controller check here stays: the hook reduces how often it trips, it does not replace the gate.
